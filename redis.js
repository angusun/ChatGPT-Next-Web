...
const host = process.env.REDIS_HOST; // redis服务地址
const port = process.env.REDIS_PORT; // redis服务端口
const db = process.env.REDIS_DATABASE; // redis数据库
...
class Redis {
  ...
  async get(key) {
    return new Promise((resolve, reject) => {
      this.redisClient.get(key, { timeout: 500 }, (err, result) => {
        if (err) {
          reject(false);
        }
        resolve(result);
